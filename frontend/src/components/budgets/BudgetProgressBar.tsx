    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
      {budgets.map((b) => {
        ...
        return (
          <div
            key={b.id}
            className={`p-5 bg-slate-900/90 border rounded-2xl shadow-lg relative ${
          ...
          >
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                ...
                />
                <h4 className="font-bold text-white text-base">{b.category.name}</h4>
              </div>
              <div className="flex items-center gap-2">
                {b.isExceeded ? (
                  <span className="inline-flex items-center gap-1 text-xs font-semibold px-2.5 py-0.5 rounded-full bg-rose-500/10 border border-rose-500/30 text-rose-400">
                    <AlertTriangle className="w-3 h-3" />
                    Exceeded by {formatCurrency(b.exceededAmount, currency)}
                  </span>
                ) : (
                  <span className="inline-flex items-center gap-1 text-xs font-semibold px-2.5 py-0.5 rounded-full bg-emerald-500/10 border border-emerald-500/30 text-emerald-400">
