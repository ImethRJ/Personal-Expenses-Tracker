  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5">
      {subscriptions.map((sub) => (
        <div
          key={sub.id}
          className="p-4 sm:p-5 bg-slate-900/90 border border-slate-800/80 rounded-2xl shadow-lg relative flex flex-col justify-between group hover:border-indigo-500/30 transition-all"
        >
          <div>
            <div className="flex items-center justify-between mb-3">
              <h3 className="font-bold text-white text-base tracking-tight truncate max-w-[160px]">{sub.name}</h3>
            ...
            <div className="flex items-center gap-2 text-xs text-slate-400 mt-3 pt-3 border-t border-slate-800">
              ...
              <span>Next renewal: <strong className="text-slate-200 font-medium">{formatDate(sub.nextPaymentDate)}</strong></span>
            </div>
          </div>
          ...
          <div className="flex items-center justify-end gap-2 mt-4 pt-3 border-t border-slate-800/60">
            <button
              onClick={() => onEdit(sub)}
              className="p-2 rounded-lg text-slate-400 hover:text-indigo-400 hover:bg-slate-800 active:scale-95 transition-all min-w-[36px] min-h-[36px] flex items-center justify-center"
            >
              <Edit2 className="w-4 h-4" />
            </button>
            <button
              onClick={() => onDelete(sub)}
              className="p-2 rounded-lg text-slate-400 hover:text-rose-400 hover:bg-slate-800 active:scale-95 transition-all min-w-[36px] min-h-[36px] flex items-center justify-center"
            >
